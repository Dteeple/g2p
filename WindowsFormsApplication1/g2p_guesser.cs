...
namespace WindowsFormsApplication1
{
    public class gToPGuesser
    {
        ...
        public static Dictionary<string, Dictionary<string, int>> mappingScores = Newtonsoft.Json.JsonConvert.DeserializeObject<Dictionary<string, Dictionary<string, int>>>(File.ReadAllText(@"wts_updated.json", Encoding.UTF8));





        public static void notMain(string[] args) // Not actually used by Forms application
        {
        ...
        public static void newpair(string orthin, string pronout)
        {
            // Iterates through mappingScores, searching for matches to new orthin-pronout pair
            // Any matches will have lcs wt augmented (by 1?), added back to JSON file
            // Full orthin-pronout mapping will receive a score/augmentation of 50
            Dictionary<string, Dictionary<string, int>> mappingScoresToUpdate = Newtonsoft.Json.JsonConvert.DeserializeObject<Dictionary<string, Dictionary<string, int>>>(File.ReadAllText(@"wts_updated.json", Encoding.UTF8));
            if (mappingScoresToUpdate.ContainsKey(orthin))
            {
                if (mappingScoresToUpdate[orthin].ContainsKey(pronout))
                {
                    mappingScoresToUpdate[orthin][pronout] += 50;
                }
                else
                {
                    mappingScoresToUpdate[orthin].Add(pronout, 50);
                }
            }
            else
            {
                //Adds whole new entry
                mappingScoresToUpdate.Add(orthin, new Dictionary<string, int> { { pronout, 50 } });

            }
            //Update JSON wts file
            //string json = Newtonsoft.Json.JsonConvert.SerializeObject(mappingScoresToUpdate);
            //System.IO.File.WriteAllText(@"wts_updated.json", json, Encoding.UTF8);
            // To get the keys alone, use the Keys property.
            //Dictionary<string, Dictionary<string, int>>.KeyCollection keyColl = mappingScoresToUpdate.Keys;
            List<string> keyList = new List<string>(mappingScoresToUpdate.Keys);
            for (int i = 0; i < keyList.Count; i++)
            {
                var o = keyList[i];
                string lcsOrth;
                int lenO = LongestCommonSubstring(orthin, o, out lcsOrth);
                if (lenO > 0)
                {
                    List<string> pronList = new List<string>(mappingScoresToUpdate[o].Keys);
                    for (int j = 0; j < pronList.Count; j++)
                    {
                        var p = pronList[j];
                        string lcsPron;
                        int lenP = LongestCommonSubstring(pronout, p, out lcsPron);
                        if (lenP > 0)
                        {
                            if (mappingScoresToUpdate.ContainsKey(lcsOrth))
                            {
                                if (mappingScoresToUpdate[lcsOrth].ContainsKey(lcsPron))
                                {
                                    mappingScoresToUpdate[lcsOrth][lcsPron] += 1;
                                }
                                else
                                {
                                    mappingScoresToUpdate[lcsOrth].Add(lcsPron, 1);
                                }
                            }
                            else
                            {
                                //Adds whole new entry
                                mappingScoresToUpdate.Add(lcsOrth, new Dictionary<string, int> { { lcsPron, 1 } });
